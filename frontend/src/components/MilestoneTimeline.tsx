  ...
  if (milestones.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-gray-500">No milestones yet. Create one to get started!</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative pl-8">
        {/* Vertical line */}
        <div className="absolute left-2 top-0 bottom-0 w-1 bg-gradient-to-b from-blue-400 to-emerald-400 rounded-full" />
        ...
        <div className="space-y-6">
          {sortedMilestones.map((milestone, index) => {
            ...
            const isOverdue =
              dueDate < new Date() && milestone.status !== 'completed' && milestone.status !== 'delayed';

            return (
              <motion.div key={milestone.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                {/* Timeline dot */}
                <div className="absolute -left-4.5 top-2 w-5 h-5 rounded-full border-4 border-white bg-blue-500 shadow-md" />
      ...
      <motion.div
        ...
        className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3 pt-4 border-t border-gray-200"
      >
